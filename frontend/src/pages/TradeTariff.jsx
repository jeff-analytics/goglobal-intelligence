...
export default function TradeTariff({ dashboard, markets, onReload, onGoSetup }) {
  ...
  return <div className="page-stack">
    <PageHeader title={t('tradeSuppliers')} actions={<><Button icon={Download} variant="secondary" disabled={!rows.length} onClick={()=>downloadCsv(`goglobal-trade-${project.id}.csv`, exportRows)}>CSV</Button><AiRecoveryAction project={project} scope="trade" markets={selectedMarket?[selectedMarket]:[]} onComplete={async()=>{await onReload(project.id);await loadAiEvidence()}} label={locale==='zh'?'AI 补全当前市场':'AI recover current market'}/><Button icon={RefreshCw} variant="primary" loading={running} disabled={!project.markets?.length} onClick={refreshAll}>{locale==='zh'?'刷新市场':'Refresh markets'}</Button></>} />
